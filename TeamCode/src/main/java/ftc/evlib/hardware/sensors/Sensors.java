...
public class Sensors {
    ...
    public static DigitalSensor digitalFromAnalogGreater(final AnalogSensor analogSensor, final double threshold) {
        return new DigitalSensor() {
            @Override
            public Boolean getValue() {
    ...
    public static DigitalSensor digitalFromAnalogLess(final AnalogSensor analogSensor, final double threshold) {
        return new DigitalSensor() {
            @Override
            public Boolean getValue() {
                return analogSensor.getValue() <= threshold;
...
//        return new SingleLineSensorArray(i2cDevice, i2cAddr, isInverted);
    ...
    /**
     * Sets the time a gyro takes to initialize
     *
     * @param hardwareMap    the hardware map to get the gyro from
     * @param gyroSensorName the gyro's name in the hardwareMap
     * @param gyroInitTime   the time to wait for the gyro to calibrate
     * @return the wrapped GyroSensor
     */
    public static GyroSensor gyroInitTime(HardwareMap hardwareMap, String gyroSensorName, Time gyroInitTime) {
        return new GyroInitFixer(hardwareMap.gyroSensor.get(gyroSensorName), (long) gyroInitTime.milliseconds());
    }

    /**
     * Gets an analog sensor from the hardwareMap and wraps it with a CalibratedLineSensor
     *
     * @param hardwareMap              the hardware map to get the sensor from
     * @param calibratedLineSensorName the name of the device in the hardwareMap
     * @return the created CalibratedLineSensor
     */
    public static CalibratedLineSensor calibratedLine(HardwareMap hardwareMap, String calibratedLineSensorName) {
        return new CalibratedLineSensor(analog(hardwareMap, calibratedLineSensorName));
    }

    public static SpikeDetector spikeDetector(HardwareMap hardwareMap, String rawSensorName, double spikeThreshold, int numLongReadings, int numShortReadings) {
        return new SpikeDetector(analog(hardwareMap, rawSensorName), spikeThreshold, numLongReadings, numShortReadings);
    }

    public static DigitalSensor inv(final DigitalSensor digitalSensor) {
        return new DigitalSensor() {
            @Override
            public Boolean getValue() {
